...
export const handleSubmitPostMenu = async (c: Context) => {
  ...
  if (perms?.includes("posts") || perms?.includes("all")) {
    return c.json({
      showForm: {
        name: "submitModPostForm", // We will link this in devvit.json later
        form: {
          title: "Submit a post",
          acceptLabel: "Post",
          cancelLabel: "Cancel",
          fields: [
            ...
            { name: "bodyP", label: "Body", type: "paragraph", required: true },
